...
(defVau (the ck value) env
  #|Return VALUE if VALUE match CHK, signals an error otherwise.
   |
   |$(fn chk value)
   |$(type macro)
   |$(derivation (let1 (value (eval value env)) (apply %check (list value chk) env) value)) 
   |#
  (let1 (value (eval value env)) (apply %check (list value ck) env) value) )
