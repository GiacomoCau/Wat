...
(defMacro (defEnv name . bindings)
  #|($nm name . bindings)
   |($nm name obj . bindings)
   |(type macro)
   |
   |(syntax bindings (attribute value . bindings))
   |(syntax attribute (or Symbol Keyword String))
...
(def caseMacro
  ...
  (makeMacro
    (vau forms #ignore
      (list 'makeMacro (list* 'caseVau forms)) )))
...
(defVau %` (x) env
  (defCase\ qq
    ( ((('%,@ x) . y) #f . d) (log 1) (append (map (\ (x) (list '%,@ x)) (apply** qq (list x) d)) (apply** qq y #f d)) )
    ( ((('%,@ x) . y) . d)    (log 2) (append (eval x env) (apply** qq y d)) )
    ( ((('%, x) . y) #f . d)  (log 3) (append (map (\ (x) (list '%, x))  (apply** qq (list x) d)) (apply** qq y #f d)) )
    ( (('%, x) #f . d)        (log 4) (list '%, (apply** qq x d)) )
    ( (('%, x))               (log 5) (eval x env) )
    ( (('%` x) . d)           (log 6 x d) (list '%` (apply** qq x #f d)) )
    ( ((x . y) . d)           (log 7 x y d) (log (cons (apply** qq x d) (apply** qq y d))) )
    ( (x . d)                 (log 8 x) x)
    (else                     (log 9) (errror "qui non deve arrivare")) )
  (qq x))
