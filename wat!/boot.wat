...
(def check
  #|Return the lenght of FORM if match CHECK, signals an error otherwise.
   |
   |$(fn form check)
   |$(type fexpr)
   |$(derivation ((\ ((#: check value)) (len value)) form))
   |$(derivation (let ( (value (eval form env)) (chk (%evalChk check env)) ) (@check vm value chk))) 
   |#
  %check)
...
(def :
  #|Return VALUE if VALUE match CHECK, signals an error otherwise.
   |
   |$(fn check form . forms)
   |$(type macro)
