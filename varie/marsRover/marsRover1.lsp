...
(defVau obje bindings env
  (def env (newEnv env))
  (env :this env) ;; anaphoric this
  (forEach (\ ((name . #ignore)) (env name #inert)) bindings)
  (forEach (\ ((name . forms))
    (if (cons? name)
      (env (car name) (eval (list* '\ (cdr name) forms) env)) ;; method
      (env name (eval (cons 'begin forms) env)) ))
    bindings )
  env )

(defVau clse bindings env
  (\ args 
    (def obj (eval (cons 'obje bindings) env))
    ((rec\ (loop args) (if (null? args) obj (let1 ((name value . args) args) (obj name value) (loop args)))) args) ))
  
(def\ (newe cls . args) (apply cls args)) 

(let ()
  (def A (clse (a 1) (B (clse (b 2) ((f c) (+ a b c))))))
  (def objA (newe A))
  (def objB (newe (objA :B) :b 5))
  (assert ((objB :f) 4) 10)
)
...
(def pl (newe Plateau :mxX 5 :mxY 5))
(def rr (newe (pl :Rover) :x 1 :y 2 :d "N"))
((rr :cmd) "LMLMLMLMM")
((rr :chk) 1 3 "N")
